...
namespace ProtocolConvertor_Configurator
{
    class FTDI_FT4222
    {
        ...
        private static FTDI.FT_DEVICE_INFO_NODE devInfo = new FTDI.FT_DEVICE_INFO_NODE();
        private static IntPtr ftHandle = new IntPtr();
        private static FTDI.FT_STATUS ftStatus = 0;
        private static FT4222_STATUS ft42Status = 0;

        public static void SingleSend(ref List<byte> configurationString)
        {
            configurationString.Add(0);

            ushort sizeTransferred = 0;

            byte[] writeBuf = configurationString.ToArray();
            /* readBuf will receive the same values so that it has the same size */
            byte[] readBuf = configurationString.ToArray();
            ...
            else
            {
                ...
                Console.WriteLine("\n W:[");
                foreach (byte b in writeBuf)
                    Console.Write("{0:X}, ", b);
                Console.WriteLine("]");

                Console.WriteLine("Read/Write OK, sizeTransferred: {0} out of {0}", sizeTransferred, writeBuf.Length);
            }
        }
